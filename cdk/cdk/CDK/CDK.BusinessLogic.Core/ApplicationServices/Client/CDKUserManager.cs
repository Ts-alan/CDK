...
{
    public class CDKUserManager : UserManager<User, long>, ICDKUserManager
    {
        public CDKUserManager(ICDKUserStore store)
            : base(store)
        {
        }

        public static CDKUserManager Create(IdentityFactoryOptions<CDKUserManager> options, IOwinContext context)
        {
            var manager = new CDKUserManager(new UserStore(context.Get<UnitOfWork>()));
            ...
            if (dataProtectionProvider != null)
            {
                manager.UserTokenProvider =
                    new DataProtectorTokenProvider<User, long>(dataProtectionProvider.Create("ResetPasswordPurpose"))
                    {
