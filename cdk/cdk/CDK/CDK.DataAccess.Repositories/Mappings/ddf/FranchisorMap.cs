namespace CDK.DataAccess.Repositories.Mappings
{
    ...
    public class FranchisorMap : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<Franchisor>
    {
        public FranchisorMap()
            : this("ddf")
        {
        }

        public FranchisorMap(string schema)
        {
            ToTable(schema + ".Franchisor");
            HasKey(x => x.Id);
            ...
            HasMany(t => t.UnitAgentOffices).WithMany(t => t.Franchisors).Map(m =>
            {
                m.ToTable("OfficeFranchisor", "ddf");
                m.MapLeftKey("FranchisorId");
