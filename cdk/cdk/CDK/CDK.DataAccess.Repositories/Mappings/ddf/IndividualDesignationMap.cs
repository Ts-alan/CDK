namespace CDK.DataAccess.Repositories.Mappings
{
    ...
    public class IndividualDesignationMap : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<IndividualDesignation>
    {
        public IndividualDesignationMap()
            : this("ddf")
        {
        }

        public IndividualDesignationMap(string schema)
        {
            ToTable(schema + ".IndividualDesignation");
            HasKey(x => x.Id);
            ...
            HasMany(t => t.UnitAgents).WithMany(t => t.IndividualDesignations).Map(m =>
            {
                m.ToTable("AgentIndividualDesignation", "ddf");
                m.MapLeftKey("IndividualDesignationId");
