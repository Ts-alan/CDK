namespace CDK.DataAccess.Repositories.Mappings
{
    ...
    public class BasementTypeMap : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<BasementType>
    {
        public BasementTypeMap()
            : this("ddf")
        {
        }

        public BasementTypeMap(string schema)
        {
            ToTable(schema + ".BasementType");
            HasKey(x => x.Id);
            ...
            HasMany(t => t.Buildings).WithMany(t => t.BasementTypes).Map(m =>
            {
                m.ToTable("BuildingBasementType", "ddf");
                m.MapLeftKey("BasementTypeId");
