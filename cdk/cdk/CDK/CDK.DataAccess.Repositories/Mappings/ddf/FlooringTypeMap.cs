namespace CDK.DataAccess.Repositories.Mappings
{
    ...
    public class FlooringTypeMap : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<FlooringType>
    {
        public FlooringTypeMap()
            : this("ddf")
        {
        }

        public FlooringTypeMap(string schema)
        {
            ToTable(schema + ".FlooringType");
            HasKey(x => x.Id);
            ...
            HasMany(t => t.Units).WithMany(t => t.FlooringTypes).Map(m =>
            {
                m.ToTable("UnitFlooringType", "ddf");
                m.MapLeftKey("FlooringTypeId");
