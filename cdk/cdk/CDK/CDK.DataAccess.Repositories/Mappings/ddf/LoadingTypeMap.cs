namespace CDK.DataAccess.Repositories.Mappings
{
    ...
    public class LoadingTypeMap : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<LoadingType>
    {
        public LoadingTypeMap()
            : this("ddf")
        {
        }

        public LoadingTypeMap(string schema)
        {
            ToTable(schema + ".LoadingType");
            HasKey(x => x.Id);
            ...
            HasMany(t => t.Units).WithMany(t => t.LoadingTypes).Map(m =>
            {
                m.ToTable("UnitLoadingType", "ddf");
                m.MapLeftKey("LoadingTypeId");
