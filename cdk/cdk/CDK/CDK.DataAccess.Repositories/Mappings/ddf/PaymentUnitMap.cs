namespace CDK.DataAccess.Repositories.Mappings
{
    ...
    public class PaymentUnitMap : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<PaymentUnit>
    {
        public PaymentUnitMap()
            : this("ddf")
        {
        }

        public PaymentUnitMap(string schema)
        {
            ToTable(schema + ".PaymentUnit");
            HasKey(x => x.Id);
            ...
            HasMany(t => t.Units).WithMany(t => t.PaymentUnits).Map(m =>
            {
                m.ToTable("UnitPaymentUnit", "ddf");
                m.MapLeftKey("PaymentUnitId");
