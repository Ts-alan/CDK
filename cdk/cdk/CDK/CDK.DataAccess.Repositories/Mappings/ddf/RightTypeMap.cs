namespace CDK.DataAccess.Repositories.Mappings
{
    ...
    public class RightTypeMap : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<RightType>
    {
        public RightTypeMap()
            : this("ddf")
        {
        ...
        public RightTypeMap(string schema)
        {
            ToTable(schema + ".RightType");
            HasKey(x => x.Id);
            ...
            HasMany(t => t.Units).WithMany(t => t.RightTypes).Map(m =>
            {
                m.ToTable("UnitRightType", "ddf");
                m.MapLeftKey("RightTypeId");
