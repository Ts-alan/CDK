namespace CDK.DataAccess.Repositories.Mappings
{
    ...
    public class AmperageMap : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<Amperage>
    {
        public AmperageMap()
            : this("ddf")
        {
        ...
        public AmperageMap(string schema)
        {
            ToTable(schema + ".Amperage");
            HasKey(x => x.Id);
            ...
            HasMany(t => t.Buildings).WithMany(t => t.Amperages).Map(m =>
            {
                m.ToTable("BuildingAmperage", "ddf");
                m.MapLeftKey("AmperageId");
