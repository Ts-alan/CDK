namespace CDK.DataAccess.Repositories.Mappings
{
    ...
    public class ArchitecturalStyleMap : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<ArchitecturalStyle>
    {
        public ArchitecturalStyleMap()
            : this("ddf")
        {
        }

        public ArchitecturalStyleMap(string schema)
        {
            ToTable(schema + ".ArchitecturalStyle");
            HasKey(x => x.Id);
            ...
            HasMany(t => t.Buildings).WithMany(t => t.ArchitecturalStyles).Map(m =>
            {
                m.ToTable("BuildingArchitecturalStyle", "ddf");
                m.MapLeftKey("ArchitecturalStyleId");
